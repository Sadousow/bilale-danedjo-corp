...
const features = [
  { icon: Truck, title: "Livraison rapide", desc: "Livraison à Conakry et dans tout le pays sous 24 à 72 heures." },
  { icon: ShieldCheck, title: "Qualité garantie", desc: "Produits sélectionnés rigoureusement chez des fournisseurs de confiance." },
  { icon: MessageCircle, title: "Service WhatsApp", desc: "Commandez directement par WhatsApp, conseil personnalisé." },
  { icon: BadgePercent, title: "Prix justes", desc: "Tarifs compétitifs pour particuliers, boutiques et professionnels." },
];
    ...
    <>
      ...
      <section className="py-16 sm:py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Stagger className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6" staggerDelay={0.1}>
            {categories.map((cat) => (
              <StaggerItem key={cat.key}>
                <Link
                  href={`/produits?cat=${cat.key}`}
                  className="group block bg-white border border-slate-200 rounded-2xl p-8 hover:shadow-xl hover:-translate-y-1 hover:border-brand-gold transition-all duration-300 h-full"
                >
                  ...
                  <h3 className="mt-5 font-display font-bold text-xl text-brand-blue">{cat.label}</h3>
      ...
      <section className="py-16 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Stagger className="mt-10 grid grid-cols-2 lg:grid-cols-4 gap-5" staggerDelay={0.06}>
            {popular.map((p) => (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 text-white">
              <Reveal className="max-w-2xl">
                ...
                <h2 className="font-display text-3xl sm:text-4xl font-bold">Promotions du moment</h2>
                <p className="mt-4 text-slate-200">Profitez de nos prix spéciaux sur une sélection de produits.</p>
              </Reveal>
              <Reveal delay={0.1}>
                <Link
                  href="/promotions"
            ...
            <Stagger className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5" staggerDelay={0.08}>
              {promos.map((p) => (
                <StaggerItem key={p.id}>
                  <ProductCard product={p} />
                </StaggerItem>
      ...
      <section className="py-16 sm:py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeading eyebrow="Pourquoi nous choisir" title="Un partenaire fiable au quotidien" />

          <Stagger className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" staggerDelay={0.08}>
            {features.map((feat) => {
              const Icon = feat.icon;
              return (
                <StaggerItem key={feat.title}>
                  <div className="bg-white border border-slate-200 rounded-xl p-6 hover:shadow-lg hover:border-brand-gold/40 hover:-translate-y-1 transition-all duration-300 h-full">
                    <div className="w-12 h-12 rounded-lg bg-brand-gold/10 text-brand-gold flex items-center justify-center mb-4">
                      <Icon className="w-6 h-6" strokeWidth={1.75} />
                    </div>
                    <h3 className="font-display font-bold text-lg text-brand-blue">{feat.title}</h3>
      ...
      <section className="py-16 sm:py-20 bg-brand-blue-dark text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Reveal>
            <h2 className="font-display text-3xl sm:text-4xl font-bold">Prêt à passer votre commande ?</h2>
          ...
          <Reveal delay={0.15}>
            <a
              ...
              className="mt-8 inline-flex items-center gap-2 bg-brand-gold hover:bg-brand-gold-dark text-white px-8 py-4 rounded-md font-semibold shadow-lg transition-all hover:scale-105 active:scale-95"
            >
